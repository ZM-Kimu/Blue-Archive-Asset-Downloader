...
internal sealed class PrimitiveMemberSignatureResolver
{
    ...
    public PrimitiveMemberSignatureResolver(
        MetadataModel model,
        ...
        string? declaringTypeName)
    {
        ...
        foreach (var field in rows)
        {
            ...
            var backingName = ResolutionUtilities.BackingFieldPropertyName(field.Name);
            if (backingName is not null && propertyTypeByName.TryGetValue(backingName, out var propertyType))
                inferredType = propertyType;
            else if (ResolutionUtilities.ConventionalPropertyCandidate(field.Name) is { } conventionalName &&
            ...
            resolved.Add(new ResolvedFieldModel(
                field,
                ResolutionUtilities.SanitizeIdentifier(field.Name, $"field_{field.Index}"),
                resolvedTypeName,
                modifiers,
                accessibility));
        }
        ...
        foreach (var method in rows)
        {
            var parameters = _descriptors.GetParameters(method)
                .Select((parameter, index) => new ResolvedParameterModel(
                    parameter,
                    ResolutionUtilities.SanitizeIdentifier(parameter.Name, $"param_{index}"),
    ...
    private static ExportMemberAccessibility ResolveFieldAccessibility(FieldDefinition field)
    {
        ...
        if (ResolutionUtilities.BackingFieldPropertyName(field.Name) is not null ||
    ...
    private static IReadOnlyList<string> ResolveFieldModifiers(FieldDefinition field)
    {
        ...
        var modifier = ResolutionUtilities.AccessibilityModifier(accessibility);
        ...
        IReadOnlyDictionary<uint, string> typeNameMap)
    {
        if (field.Name == "value__")
        {
            ...
            return ResolutionConstants.AliasToSystemType.GetValueOrDefault(underlyingType, underlyingType);
        }
    ...
    public string[] ResolveTypeGenericParameterNames(TypeDefinition type)
    {
        ...
        for (var i = 0; i < container.Value.TypeArgumentCount; i++)
        {
            ...
            if (parameterIndex >= 0 && parameterIndex < _model.GenericParameters.Count)
            {
                ...
                var safeName = ResolutionUtilities.SanitizeIdentifier(parameter.Name, $"T{i}");
