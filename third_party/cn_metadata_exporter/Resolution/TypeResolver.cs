...
internal sealed class TypeResolver
{
    ...
    public TypeResolver(MetadataModel model, TypeDescriptorIndex descriptors)
    {
        _model = model;
    ...
    public string ResolveTypeName(uint typeIndex, IReadOnlyDictionary<uint, string> typeNameMap, string? fallback = null)
    {
        if (typeNameMap.TryGetValue(typeIndex, out var resolvedType))
        {
            ...
            {
                return ResolutionUtilities.SimplifyTypeName(fallback!);
            }

            return ResolutionUtilities.SimplifyTypeName(resolvedType);
        }

        return ResolutionUtilities.FormatType(typeIndex, typeNameMap, fallback);
    }
    ...
    private static Dictionary<uint, string> BuildDirectTypeNameMap(IReadOnlyList<TypeDefinition> types)
    {
        ...
        foreach (var pair in grouped)
            resolved[pair.Key] = ResolutionUtilities.ChooseTypeName(pair.Value) ?? $"Type_0x{pair.Key:X8}";
        return resolved;
    }
        ...
        IReadOnlyList<ParameterDefinition> parameters)
    {
        ...
        foreach (var method in methods)
        {
            ...
            var methodParams = ResolutionUtilities.GetMethodParameters(method, parameters);
    ...
    private static Dictionary<uint, string> BuildHeuristicPrimitiveTypeMap(IReadOnlyList<FieldDefinition> fields)
    {
        ...
        foreach (var field in fields)
        {
            ...
            if (string.Equals(field.Name, "value__", StringComparison.Ordinal))
            {
             ...
             if (ResolutionUtilities.LooksLikeReferenceSemanticName(field.Name) ||
                ResolutionUtilities.TrySingularizeCollectionName(field.Name, out _))
            {
                continue;
            }

            if (ResolutionConstants.IntLikeFieldNames.Contains(field.Name))
                Vote(field.TypeIndex, "System.Int32", 3);

            if (ResolutionConstants.BoolLikeFieldNames.Contains(field.Name))
        ...
        IReadOnlyDictionary<uint, string> knownTypeNames)
    {
        ...
        foreach (var field in fields)
        {
            ...
            if (knownTypeNames.TryGetValue(field.TypeIndex, out var existingTypeName) &&
                ...
                !ResolutionUtilities.IsPrimitiveOrVoidTypeName(existingTypeName) &&
                !ResolutionUtilities.LooksLikeReferenceSemanticName(field.Name))
            {
                continue;
            }
    ...
    private static bool ShouldPreferFallback(string resolvedType, string? fallback)
    {
        ...
        if (ResolutionUtilities.IsPrimitiveOrVoidTypeName(resolvedType) &&
            !ResolutionUtilities.IsPrimitiveOrVoidTypeName(fallback))
        {
            return true;
        }
    ...
    private static Dictionary<uint, string> BuildHeuristicParameterTypeMap(IReadOnlyList<ParameterDefinition> parameters, bool includeIntLike)
    {
        ...
        {
            if (ResolutionConstants.ByteArrayParameterNames.Contains(parameter.Name))
                Vote(parameter.TypeIndex, "System.Byte[]", 4);

            if (ResolutionConstants.BoolLikeParameterNames.Contains(parameter.Name))
                Vote(parameter.TypeIndex, "System.Boolean", 3);

            if (includeIntLike && ResolutionConstants.IntLikeParameterNames.Contains(parameter.Name))
                Vote(parameter.TypeIndex, "System.Int32", 3);

            if (ResolutionConstants.FloatLikeParameterNames.Contains(parameter.Name))
        ...
        IReadOnlyDictionary<uint, string> knownTypeNames)
    {
        ...
        void RememberMemberType(int declaringTypeIndex, string memberName, string typeName)
        {
            ...
            foreach (var key in ResolutionUtilities.ExpandMemberNameKeys(memberName, typeName))
            {
        ...
        foreach (var method in methods)
        {
                ...
                vectorElementTypesByDeclaringType.TryGetValue(method.DeclaringTypeIndex, out var vectorTypes))
            {
                ...
                if (vectorTypes.TryGetValue(vectorName, out var elementCandidates) && elementCandidates.Count == 1)
                    inferred[methodParams[1].TypeIndex] = ResolutionUtilities.NormalizeTypeCandidate(elementCandidates.First()) + "[]";
            }
        ...
        IReadOnlyList<FieldDefinition> fieldRows)
    {
        ...
        foreach (var method in methodRows)
        {
            ...
            var iface = ResolutionUtilities.ExplicitInterfacePrefix(method.Name);
        ...
        if (hints.HasCollectionLikeMembers || isTableLike)
        {
            ...
            foreach (var method in methodRows)
            {
                ...
                if (methodParams.Count == 1 &&
                    ...
                    !method.Name.StartsWith("Get", StringComparison.Ordinal))
                {
                    vectorElementTypes[method.Name] = ResolutionUtilities.NormalizeTypeCandidate(knownReturnType);
                }
            }
        }
        ...
        IReadOnlyDictionary<uint, string> localTypeNames)
    {
        ...
        void RememberKnown(string memberName, string? typeName)
        {
            if (string.IsNullOrWhiteSpace(memberName) || string.IsNullOrWhiteSpace(typeName))
                return;
            if (typeName!.StartsWith("Type_0x", StringComparison.Ordinal) ||
                ResolutionUtilities.IsPrimitiveOrVoidTypeName(typeName))
            {
                return;
            }

            singularMemberTypes[memberName] = ResolutionUtilities.NormalizeTypeCandidate(typeName);
        }
        ...
        {
            if (!ResolutionUtilities.TrySingularizeCollectionName(memberName, out var singularName))
                return;
        ...
        {
            if (!ResolutionUtilities.TrySingularizeCollectionName(memberName, out _) ||
                !ResolutionUtilities.LooksLikeReferenceSemanticName(memberName))
            {
                return;
            }
        ...
        foreach (var field in fieldRows)
        {
            if (ResolutionUtilities.BackingFieldPropertyName(field.Name) is { } backingProperty)
            {
    ...
    {
        var inferred = new Dictionary<uint, string>();
        var parsed = ResolutionUtilities.ParseGenericType(interfaceName);
