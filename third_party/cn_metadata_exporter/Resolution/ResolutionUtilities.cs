...
internal static class ResolutionUtilities
{
    ...
    public static ExportMemberAccessibility MethodAccessibility(ushort flags) => (flags & ResolutionConstants.MethodAttrMemberAccessMask) switch
    {
        ResolutionConstants.MethodAttrPrivate => ExportMemberAccessibility.Private,
        ResolutionConstants.MethodAttrFamAndAssem => ExportMemberAccessibility.PrivateProtected,
        ResolutionConstants.MethodAttrAssembly => ExportMemberAccessibility.Internal,
        ResolutionConstants.MethodAttrFamily => ExportMemberAccessibility.Protected,
        ResolutionConstants.MethodAttrFamOrAssem => ExportMemberAccessibility.ProtectedInternal,
        ResolutionConstants.MethodAttrPublic => ExportMemberAccessibility.Public,
    ...
    public static string? ChooseTypeName(IEnumerable<string> candidates)
    {
        ...
        foreach (var pair in ResolutionConstants.SystemAliases)
        {
        ...
        return chosen is null ? null : ResolutionConstants.SystemAliases.GetValueOrDefault(chosen, chosen);
    }
    ...
    public static string NormalizeTypeCandidate(string candidate)
    {
        ...
        if (ResolutionConstants.AliasToSystemType.TryGetValue(candidate, out var fullName))
            return fullName;

        if (candidate.EndsWith("[]", StringComparison.Ordinal))
        {
            var elementName = candidate[..^2];
            if (ResolutionConstants.AliasToSystemType.TryGetValue(elementName, out var fullElementName))
    ...
    public static string SimplifyTypeName(string typeName)
    {
        ...
        foreach (var pair in ResolutionConstants.SystemAliases.OrderByDescending(pair => pair.Key.Length))
