...
internal sealed class RelationshipResolver
{
    ...
    public RelationshipResolver(MetadataModel model, TypeResolver typeResolver, TypeDescriptorIndex descriptors)
    {
        _model = model;
    ...
    public string? ResolveDeclaringType(TypeDefinition type, IReadOnlyDictionary<uint, string> typeNameMap)
    {
        ...
        return ResolutionUtilities.IsPrimitiveOrVoidTypeName(ownerName) ? null : ownerName;
    }
    ...
    public IReadOnlyList<string> ResolveTypeModifiers(TypeDefinition type)
    {
        ...
        {
            (flags & ResolutionConstants.TypeAttrVisibilityMask) switch
            {
                ResolutionConstants.TypeAttrPublic or ResolutionConstants.TypeAttrNestedPublic => "public",
                ResolutionConstants.TypeAttrNestedPrivate => "private",
                ResolutionConstants.TypeAttrNestedFamily => "protected",
                ResolutionConstants.TypeAttrNestedAssembly => "internal",
                ResolutionConstants.TypeAttrNestedFamAndAssem => "private protected",
                ResolutionConstants.TypeAttrNestedFamOrAssem => "protected internal",
                _ => "internal",
            }
        };

        if ((flags & ResolutionConstants.TypeAttrInterface) != 0)
        {
        ...
        var isAbstract = (flags & ResolutionConstants.TypeAttrAbstract) != 0;
        var isSealed = (flags & ResolutionConstants.TypeAttrSealed) != 0;
    ...
    public IReadOnlyList<string> ResolveMethodModifiers(MethodDefinition method, bool? confirmedOverride = null)
    {
        var modifiers = new List<string>
        {
            ResolutionUtilities.AccessibilityModifier(ResolveMethodAccessibility(method))
        };

        if ((method.Flags & ResolutionConstants.MethodAttrStatic) != 0)
            modifiers.Add("static");

        var isAbstract = (method.Flags & ResolutionConstants.MethodAttrAbstract) != 0;
        var isVirtual = (method.Flags & ResolutionConstants.MethodAttrVirtual) != 0;
        var isFinal = (method.Flags & ResolutionConstants.MethodAttrFinal) != 0;
        ...
        var codeType = (ushort)(method.ImplFlags & ResolutionConstants.MethodImplCodeTypeMask);
        if ((method.ImplFlags & ResolutionConstants.MethodImplInternalCall) != 0 ||
            codeType is ResolutionConstants.MethodImplNative or ResolutionConstants.MethodImplRuntime)
        {
    ...
    public ExportMemberAccessibility ResolveMethodAccessibility(MethodDefinition method)
        => ResolutionUtilities.MethodAccessibility(method.Flags);
    ...
    public ExportMemberAccessibility ResolvePropertyAccessibility(TypeDefinition type, PropertyDefinition property)
    {
        ...
        return accessors
            ...
            .OrderByDescending(accessibility => ResolutionUtilities.AccessModifierRank(ResolutionUtilities.AccessibilityModifier(accessibility)))
            .First();
    }
    ...
    public IReadOnlyList<string> ResolvePropertyModifiers(TypeDefinition type, PropertyDefinition property, bool? confirmedOverride = null)
    {
        ...
        baseModifiers[0] = ResolutionUtilities.AccessibilityModifier(ResolvePropertyAccessibility(type, property));
    ...
    public IReadOnlyList<string> ResolveEventModifiers(TypeDefinition type, EventDefinition evt, bool? confirmedOverride = null)
    {
        ...
        var baseModifiers = ResolveMethodModifiers(accessors[0], confirmedOverride).ToList();
        baseModifiers[0] = ResolutionUtilities.AccessibilityModifier(ResolveEventAccessibility(type, evt));
    ...
    public bool? ResolveOverride(TypeDefinition type, MethodDefinition method, IReadOnlyDictionary<uint, string> typeNameMap)
    {
        ...
        if ((method.Flags & ResolutionConstants.MethodAttrVirtual) == 0)
        {
            _overrideByMethodIndex[method.Index] = EncodeOverrideCache(false);
            return false;
        }
        if ((method.Flags & ResolutionConstants.MethodAttrNewSlot) != 0)
        {
            _overrideByMethodIndex[method.Index] = EncodeOverrideCache(false);
            return false;
        }
        ...
        IReadOnlyDictionary<uint, string> typeNameMap)
    {
        ...
        foreach (var explicitInterface in methodRows
                     .Select(method => ResolutionUtilities.ExplicitInterfacePrefix(method.Name))
        ...
        IReadOnlyDictionary<uint, string> typeNameMap)
    {
        ...
        foreach (var type in typeDefs)
        {
            ...
            {
                members.Add(ResolutionUtilities.MethodContractKeyForResolvedParameters(method, GetMethodParameters(method), typeNameMap));
    ...
    private IEnumerable<string> ResolveMetadataInterfaces(TypeDefinition type, IReadOnlyDictionary<uint, string> typeNameMap)
    {
        ...
        for (var i = 0; i < type.InterfaceCount; i++)
        {
            ...
            if (string.IsNullOrWhiteSpace(interfaceName) ||
                ...
                ResolutionUtilities.IsPrimitiveOrVoidTypeName(interfaceName))
            {
                continue;
            }
        ...
        IReadOnlyDictionary<uint, string> typeNameMap)
    {
        ...
        foreach (var method in methodRows)
        {
            foreach (var key in ResolutionUtilities.ExpandedMethodContractKeysForResolvedParameters(method, GetMethodParameters(method), typeNameMap))
    ...
    private TypeDefinition? InferStructuralBaseType(TypeDefinition type, IReadOnlyDictionary<uint, string> typeNameMap)
    {
        ...
        var overrideLikeMethods = IterTypeMethods(type)
            .Where(method =>
                (method.Flags & ResolutionConstants.MethodAttrVirtual) != 0 &&
                (method.Flags & ResolutionConstants.MethodAttrNewSlot) == 0)
            .ToArray();
        if (overrideLikeMethods.Length < 5)
        {
            overrideLikeMethods = IterTypeMethods(type)
                .Where(method => (method.Flags & ResolutionConstants.MethodAttrVirtual) != 0)
                .ToArray();
        }
        if (overrideLikeMethods.Length < 5)
        {
    ...
    private static string? NormalizeResolvedType(string? typeName)
    {
        ...
        if (typeName.StartsWith("Type_0x", StringComparison.Ordinal))
            return null;
        return ResolutionUtilities.NormalizeTypeCandidate(typeName!);
    }
    ...
    private static bool IsValidBaseTypeName(string? typeName)
    {
        ...
        if (typeName.StartsWith("Type_0x", StringComparison.Ordinal))
            return false;
        if (ResolutionUtilities.IsPrimitiveOrVoidTypeName(typeName))
            return false;
