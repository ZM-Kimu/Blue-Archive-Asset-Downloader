...
namespace CnMetadataExporter.Application;
...
internal sealed class ResolvedExportArtifactBuilder
{
    ...
    public ResolvedExportArtifact Build()
    {
        ...
        var typeResolver = Measure("build type-system indexes", () => new TypeResolver(_model, descriptors));

        _progress?.Stage("type index build", 3, 7);
        var typeIndex = Measure("type index build", () => new TypeIndexBuilder(_model, typeResolver, descriptors, _progress).Build());

        _progress?.Stage("relationship resolver build", 4, 7);
        var relationshipResolver = Measure("relationship build", () => new RelationshipResolver(_model, typeResolver, descriptors));

        _progress?.Stage("relationship index build", 5, 7);
        var relationshipIndex = Measure("relationship index build", () => new RelationshipIndexBuilder(_model, relationshipResolver, typeIndex, descriptors, _progress).Build());

        _progress?.Stage("member resolver build", 6, 7);
        var memberResolver = Measure("member resolver build", () => new MemberResolver(_model, typeResolver, relationshipResolver, descriptors));

        _progress?.Stage("member signature build", 7, 7);
        var memberIndex = Measure("member signature build", () => new MemberIndexBuilder(_model, memberResolver, typeIndex, relationshipIndex, _progress).Build());
        _progress?.Complete();
