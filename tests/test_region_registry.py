...
def test_default_registry_contains_all_regions() -> None:
    assert DEFAULT_REGION_REGISTRY.resolve("cn").__name__ == "CNServer"
    assert DEFAULT_REGION_REGISTRY.resolve("gl").__name__ == "GLServer"
    assert DEFAULT_REGION_REGISTRY.resolve("jp").__name__ == "JPServer"
