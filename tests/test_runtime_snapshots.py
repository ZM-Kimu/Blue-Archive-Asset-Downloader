...
def test_runtime_snapshot_retains_current_and_previous_versions(
    ...
    context = build_execution_context(tmp_path, region="cn", version="2.1.2")
    ...
    assert not (context.workspace.runtime_state / "2.1.0").exists()
    assert (context.workspace.runtime_state / "2.1.1" / "Runtime").is_dir()
    assert (context.workspace.runtime_state / "2.1.2" / "Runtime").is_dir()
...
def test_versioned_package_directories_use_the_same_retention_policy(
    ...
    assert not (context.workspace.runtime_state / "1.2.1").exists()
    assert (context.workspace.runtime_state / "1.2.2" / "Package").is_dir()
    assert (context.workspace.runtime_state / "1.2.3" / "Package").is_dir()
