...
public readonly struct PathChecksumCache
{
	...
	public PathChecksumCache(
		IAssemblyManager assemblyManager,
		IEnumerable<IGameObject> roots,
		IEnumerable<IAvatar> avatars)
	{
		this.assemblyManager = assemblyManager;
		foreach (IAvatar avatar in avatars.Distinct())
		{
			AddAvatarTOS(avatar);
		}
		foreach (IGameObject root in roots.Distinct())
		{
			AddGameObjectPathsToCacheRecursive(root, string.Empty);
		}
	}
