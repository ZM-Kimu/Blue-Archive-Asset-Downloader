...
public sealed class AudioMixerProcessor : IAssetProcessor
{
	...
	{
		string mixerIdentity = CreateMixerIdentity(mixer);
		GuidIndexTable indexToGuid = new(mixerIdentity);
		List<IAudioMixerGroupController> groups = new();

		ProcessAudioMixerGroups(mixer, indexToGuid, groups, groupGuidMap);
		ProcessAudioMixerEffects(mixer, indexToGuid, groups, virtualFile, mixerIdentity);
		...
		string mixerIdentity)
	{
		...
		foreach (IAudioMixerGroupController group in groups)
		{
			if (!groupsWithAttenuation.Contains(group))
			{
				IAudioMixerEffectController effect = virtualFile.CreateAudioMixerEffectController();
				effect.HideFlagsE = HideFlags.HideInHierarchy | HideFlags.HideInInspector;
				effect.EffectID.CopyValues(CreateAttenuationEffectGuid(mixerIdentity, group));
	...
	private static string CreateMixerIdentity(IAudioMixer mixer)
	{
		string collectionName = mixer.Collection.Name;
		return string.Concat(
			...
			collectionName,
			":",
			mixer.PathID.ToString(CultureInfo.InvariantCulture));
	}

	private static UnityGuid CreateAttenuationEffectGuid(
		string mixerIdentity,
		IAudioMixerGroupController group)
	{
		string collectionName = group.Collection.Name;
		string groupIdentity = string.Concat(
			collectionName.Length.ToString(CultureInfo.InvariantCulture),
			":",
			collectionName,
			":",
			group.PathID.ToString(CultureInfo.InvariantCulture));
		return UnityGuid.Md5Hash(
			string.Concat(mixerIdentity, ":attenuation:", groupIdentity));
	}
