...
public class EditorFormatProcessor : IAssetProcessor
{
	...
	public void Process(GameData gameData)
	{
		...
		checksumCache = processAnimationClips ? new PathChecksumCache(gameData) : null;
		IUnityObjectBase[] releaseAssets = GetReleaseAssets(gameData).ToArray();

		foreach (IUnityObjectBase asset in releaseAssets)
		{
			Convert(asset);
		}

		Parallel.ForEach(releaseAssets, ConvertAsync);
	...
	private void Convert(IUnityObjectBase asset)
	{
		switch (asset)
		{
			...
			case ISpriteAtlas spriteAtlas:
				...
				break;
			case IAnimationClip animationClip when checksumCache.HasValue:
