...
internal static class ZipExtractor
{
	...
	public static List<string> Process(IEnumerable<string> paths, FileSystem fileSystem, IGameLoadProgress? progress = null)
	{
		...
		for (int i = 0; i < inputPaths.Count; i++)
		{
			...
			progress?.Report(
				new GameLoadProgress(GameLoadProgressStage.ExtractingInputs, i + 1, inputPaths.Count)
			);
		}
		return result;
	}
