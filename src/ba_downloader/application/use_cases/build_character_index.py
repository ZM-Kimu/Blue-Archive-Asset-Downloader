...
class BuildCharacterIndexUseCase:
    ...
    def build(self, context: RuntimeContext) -> RuntimeContext:
        ...
        if active_context.region == "jp":
            ...
            active_context = active_context.with_updates(
                database_source_identity=DatabaseSourceIdentity(
                    region=active_context.region,
                    platform=active_context.platform,
                    release=active_context.version,
                    size=excel_source.size,
                    checksum=(
                        f"{excel_source.checksum.algorithm}:"
                        f"{excel_source.checksum.value}"
                    ),
                )
            )
        ...
        if schema_snapshot is not None:
            active_context = active_context.with_updates(
                schema_snapshot_root=str(schema_snapshot.root_dir)
            )
        self.cancellation.raise_if_cancelled()
