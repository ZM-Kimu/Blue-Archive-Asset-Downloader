...
from ba_downloader.lib.encryption import convert_string, create_key
from ba_downloader.lib.structure import JPResource, Resource
from ba_downloader.utils.config import Config
from ba_downloader.utils.util import UnityUtils, ZipUtils


class JPServer:
    INFO_URL = "https://prod-noticeindex.bluearchiveyostar.com/prod/index.json"
    APKPURE_URL = "https://d.apkpure.net/b/XAPK/com.YostarJP.BlueArchive?version=latest&nc=arm64-v8a&sv=24"
        ...
        if not (
            (
                apk_req := FileDownloader(
                    apk_url, request_method="get", bypass_cloudflare=True
                )
            )
